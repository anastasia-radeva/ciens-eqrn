...
EQRN_fit_seq <- function(X, y, intermediate_quantiles, interm_lvl, shape_fixed=FALSE, hidden_size=10, num_layers=1, rnn_type=c("lstm","gru"), p_drop=0,
                         intermediate_q_feature=TRUE, learning_rate=1e-4, L2_pen=0, seq_len=10, stride=1, shape_penalty=0,
  ...
  trainset <- mts_dataset(y, Xs, seq_len, scale_Y=scale_features,
                          intermediate_quantiles=intermediate_quantiles,
  ...
  if(do_validation){
    ...
    validset <- mts_dataset(y_valid, data_scaling_ex$X_scaled, seq_len, scale_Y=Y_scaling,
                            intermediate_quantiles=quant_valid,
  ...
  fit_eqrn_ts <- list(fit_nn = network, interm_lvl = interm_lvl, intermediate_q_feature = intermediate_q_feature, seq_len=seq_len, stride=stride,
...
#' @export
EQRN_predict_params_seq <- function(fit_eqrn, X, Y, intermediate_quantiles=NULL,
                                    return_parametrization=c("classical","orthogonal"),
                                    interm_lvl=fit_eqrn$interm_lvl,
                                    seq_len=fit_eqrn$seq_len,
                                    stride = if (!is.null(fit_eqrn$stride)) fit_eqrn$stride else 1,
                                    device=default_device()) {
  
  return_parametrization <- match.arg(return_parametrization)
  if (interm_lvl != fit_eqrn$interm_lvl) stop("EQRN interm_lvl does not match in train and predict.")
  ...
  if (fit_eqrn$orthogonal_gpd & return_parametrization=="classical") scales <- scales / (shapes + 1.)
  if ((!fit_eqrn$orthogonal_gpd) & return_parametrization=="orthogonal") scales <- scales * (shapes + 1.)
  
  t_idx <- testset$get_target_idx()
  return(list(scales=scales, shapes=shapes, seq_len=seq_len, target_idx=t_idx))
}
...
EQRN_predict_internal_seq <- function(fit_eqrn, X, Y, prob_lvl_predict, intermediate_quantiles, interm_lvl,
  ...
  params <- EQRN_predict_params_seq(fit_eqrn, X, Y, intermediate_quantiles,
                                    return_parametrization="classical",
                                    interm_lvl=interm_lvl, seq_len=seq_len,
                                    stride=stride, device=device)
  
  sigmas <- params$scales
  xis    <- params$shapes
  t_idx  <- params$target_idx
  
  thr <- intermediate_quantiles[t_idx]
  
  preds <- matrix(GPD_quantiles(prob_lvl_predict, interm_lvl, thr, sigmas, xis), ncol=1)
...
EQRN_excess_probability_seq <- function(val, fit_eqrn, X, Y, intermediate_quantiles,
  ...
  # --- predict tail params only at target indices given stride --------------
  params <- EQRN_predict_params_seq(
  ...
  sigmas <- as.numeric(params$scales)
  xis    <- as.numeric(params$shapes)
  t_idx  <- as.integer(params$target_idx)  # indices into rows of X (and intermediate_quantiles)
  
  # threshold aligned to predicted indices
  thr <- as.numeric(intermediate_quantiles[t_idx])
  
  # --- sanitize parameters --------------------------------------------------
  sigmas[!is.finite(sigmas)] <- NA_real_
  sigmas <- pmax(sigmas, eps_sigma)      # enforce sigma > 0
  
  xis[!is.finite(xis)] <- 0              # fallback to xi=0 if needed
  ...
  # support: z < -sigma/xi. If val beyond endpoint -> exceed prob = 0
  ...
  # --- return cropped or full-length ---------------------------------------
  if (crop_predictions) {
    return(probs)     # length = length(t_idx)
  } else {
    out_full <- rep(NA_real_, nrow(X))
    out_full[t_idx] <- probs
    return(out_full)  # length = nrow(X)
  }
}
...
mts_dataset <- torch::dataset(
  ...
  initialize = function(Y, X, seq_len, intermediate_quantiles=NULL, scale_Y=TRUE,
                        fold_separation=NULL, sample_frac=1, stride=1,
                        target = c("next","end"),
                        device=EQRN::default_device()) {
    
    target <- match.arg(target)
    
    self$seq_len <- as.integer(seq_len)
    ...
    # how many possible start positions?
    # target="next": need room for end+1 => starts in 1:(n_total - seq_len)
    # target="end" : target=end     => starts in 1:(n_total - seq_len + 1)
    n <- if (target == "next") (n_total - self$seq_len) else (n_total - self$seq_len + 1)
    if (n <= 0) stop("Not enough data for given seq_len/target definition.")
    
    if (!is.null(intermediate_quantiles)) {
      # exceedance-driven sampling: choose target indices first, then map to starts
      # define "target index" consistently with target choice:
      # target="next": y at t, window ends at t-1 => start = t - seq_len
      # target="end" : y at t, window ends at t   => start = t - seq_len + 1
      t_all <- which(c(Y) >= c(intermediate_quantiles))
      if (target == "next") {
        self$starts <- t_all - self$seq_len
      } else { # target == "end"
        self$starts <- t_all - self$seq_len + 1
      }
      
      # remove invalid starts
      self$starts <- self$starts[self$starts >= 1 & self$starts <= n]
      
      # response is excess over intermediate quantile (same indexing as response itself)
      self$response <- self$response - torch::torch_tensor(intermediate_quantiles)$reshape(list(-1,1))
      
      # apply stride on TARGET index (not start)
      if (self$stride > 1 && length(self$starts) > 0) {
        # target indices for current starts
        target_idx <- self$get_target_idx()
        
        # choose a fixed origin for the grid
        origin <- if (self$target == "next") (self$seq_len + 1L) else self$seq_len
        keep <- ((target_idx - origin) %% self$stride) == 0
      ...
      if (self$stride <= 1) {
        ...
        self$starts <- seq.int(from = 1L, to = n, by = self$stride)
      }
    }
    
    # fold separation (allow vector)
    if (!is.null(fold_separation)) {
      fs <- as.integer(fold_separation)
      bad <- unlist(lapply(fs, function(sep) (sep - self$seq_len):(sep - 1)))
      self$starts <- setdiff(self$starts, bad)
    }
    
    self$starts <- sort(unique(as.integer(self$starts)))
  },
  
  get_Y_responses = function() self$response,
  get_Y_scaling   = function() self$Y_scaling,
  
  get_target_idx = function() {
    # if target="next": target is end+1 = start + seq_len
    # if target="end" : target is end   = start + seq_len - 1
    if (self$target == "next") self$starts + self$seq_len else self$starts + self$seq_len - 1L
  },
  
  .getitem = function(i) {
    start <- self$starts[i]
    end   <- start + self$seq_len - 1L
    y_idx <- if (self$target == "next") (end + 1L) else end
    list(x = self$input_seq[start:end,], y = self$response[y_idx,])
  },
  
  .length = function() length(self$starts)
)
