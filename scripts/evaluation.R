...
# Example script for Brocken station
...
source("~/scripts/GRF_EQRN_train.R")
...
if (is.matrix(qpred_eqrn)) {
  q_vec <- as.numeric(qpred_eqrn[, 1])
} else {
  q_vec <- as.numeric(qpred_eqrn)
}

if (is.matrix(qpred_extreme_grf)) {
  q_grf_vec <- as.numeric(qpred_extreme_grf[, 1])
} else {
  q_grf_vec <- as.numeric(qpred_extreme_grf)
}
...
# -----------------------------
coverage_qrn <- mean(ts_df$y_obs <= ts_df$q_grf, na.rm = TRUE)
save(coverage_qrn, file = "~/coverage/iid_coverage_qrn_10453")

coverage <- mean(y_vec <= q_vec, na.rm = TRUE)
save(coverage, file = "~/coverage/iid_coverage_10453")
n_exceed <- sum(y_vec > q_vec, na.rm = TRUE)
n_total <- sum(is.finite(y_vec) & is.finite(q_vec))
...
# Actually it should be qs_grf and not qs_qrn, but for consistency reasons to the rnn case and easier plotting, stick to qrn
qs_qrn  <- quantile_score(ts_df$y_obs, ts_df$q_grf, levels_predict)
qs_eqrn <- quantile_score(ts_df$y_obs, ts_df$q_ext, levels_predict)

save(qs_qrn,  file = "~/coverage/iid_qs_qrn_10453")
save(qs_eqrn, file = "~/coverage/iid_qs_eqrn_10453")
...
ens_max_win <- forecasts_10453 %>%
  ...
  group_by(obs_tm) %>%
  slice(1) %>%           # take the 2nd row for each obs_tm
  ungroup()
...
p1 <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste("Window centered on peak gust at", t0),
    x = "Observation time",
    y = "Wind gust"
  ) +
  color_scale +
